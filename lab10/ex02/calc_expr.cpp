...
public:
	...
	std::string calculate() const { return std::to_string(val); }
...
std::string Expression::calculate() const
{
	auto calculate_v = [](const auto& x) { return x->calculate(); };
	return std::visit(calculate_v, expr);
}
...
std::string Bin_expr::calculate() const
{
	double l = std::stod(left.calculate());
	double r = std::stod(right.calculate());
	switch (op) {
	case '+':
...
public:
	...
	static constexpr char calculate = '=';
...
Expression primary(Token_stream& ts)
{
	...
	case '-':    // "-" Primary
		return Expression('-', Expression{0}, primary(ts));
